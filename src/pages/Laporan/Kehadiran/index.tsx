...
export default function LaporanKehadiran() {
  ...
  const mockData = [
    { id: 1, nama: "Andi Saputra", tanggal: "05 Jun 2026", clockIn: "07:45", clockOut: "16:05", status: "Hadir" },
    { id: 2, nama: "Budi Santoso", tanggal: "05 Jun 2026", clockIn: "08:10", clockOut: "16:15", status: "Terlambat" },
    { id: 3, nama: "Citra Lestari", tanggal: "05 Jun 2026", clockIn: "-", clockOut: "-", status: "Cuti" },
    { id: 4, nama: "Deni Irawan", tanggal: "04 Jun 2026", clockIn: "07:50", clockOut: "16:00", status: "Hadir" },
  ];
  ...
  const handleExport = (type: "pdf" | "excel") => {
    ...
    setTimeout(() => {
      ...
      Swal.fire({
        ...
        text: `Laporan Kehadiran telah diunduh dalam format ${type.toUpperCase()}.`,
        icon: "success",
    ...
    <>
      ...
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h2 className="text-2xl font-bold text-gray-800 dark:text-white/90">Laporan Kehadiran</h2>
            <p className="text-sm text-gray-500 dark:text-gray-400">Rekap waktu masuk, keluar, dan status absensi petugas.</p>
          </div>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                {mockData.map((row) => (
                  <tr key={row.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                    ...
                    <td className="px-6 py-4 font-medium">{row.clockIn}</td>
                    <td className="px-6 py-4 font-medium">{row.clockOut}</td>
                    <td className="px-6 py-4 text-center">
                      <span className={`inline-flex rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider ${
                        row.status === "Hadir" ? "bg-success-50 text-success-700 border border-success-200 dark:bg-success-500/10 dark:border-success-500/20 dark:text-success-400"
                        : row.status === "Terlambat" ? "bg-warning-50 text-warning-700 border border-warning-200 dark:bg-warning-500/10 dark:border-warning-500/20 dark:text-warning-400"
